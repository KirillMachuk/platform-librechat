...
/* Measured on the stand: the owner stopped a presentation run by hand at 07:55 UTC on
 * 2026-09-09; its title had been generated and billed four seconds in
 * (`db.transactions` context=title) and the chat still came out «New Chat». A user Stop
 * does NOT reach the controller's catch — `chatCompletion` swallows its own abort, so
 * the run unwinds by the success path, where `abortJob` has already deleted the job.
 * The predicate below is about the final event only; what happens to the title is
 * guarded in __tests__/titleDiscardWiring.spec.js. */
describe('shouldSkipFinalEmit — whether an ended job still sends its final', () => {
  const MINE = 1000;

  it('a Stop deleted the job: stay silent, abortJob already sent the final', () => {
    expect(shouldSkipFinalEmit(undefined, MINE)).toBe(true);
    expect(shouldSkipFinalEmit(null, MINE)).toBe(true);
  });

  it('a newer job took the stream: stay silent', () => {
