...
| Deleting a document also removes its text from search          | unit  | `api/server/services/Files/process.spec.js#deletes vector storage for a file whose embed never finished`                               | covered |
| A document deleted mid-indexing leaves nothing behind          | unit  | `api/server/services/Files/Embed/worker.spec.js#purges the vectors it just wrote when the record was deleted mid-embed`                | covered |
| A delete that could not finish is reported, not silently kept  | unit  | `api/server/services/Files/VectorDB/crud.spec.js#refuses to report success when the vector store failed: connection refused`           | covered |
