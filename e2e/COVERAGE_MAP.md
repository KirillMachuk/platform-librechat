...
| ApprovalCard (cards К1): carousel auto-advance, «Другое…» commit, Continue gate, controlled pie, ✕/external cancel | unit  | `client/src/components/Chat/Cards/__tests__/ApprovalCard.spec.tsx#keeps Continue disabled` | covered |
| DR plan card on the approval frame (К2): header ✕ = cancel marker, fixed 30s pie, refusal keeps buttons, config 0 = off | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/PlanCard.spec.tsx#a non-zero config keeps autostart ON` | covered |
