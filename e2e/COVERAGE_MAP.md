...
| The DR command chip («Исследование запущено/отменено») reaches BOTH user render paths — prod stores these commands without `content[]` | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/ActionChip.renderPaths.spec.tsx#renders on the text-only path` | covered |
| A chip (DR command / ask answers) stands aside for the editor, so the message's own Edit button is not a no-op | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/ActionChip.renderPaths.spec.tsx#yields to the editor when the message is being edited` | covered |
...
| A field that arrives late (drKind on the server save, a re-parented command) still reaches the render past React.memo | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/ActionChip.renderPaths.spec.tsx#drKind added after the first render brings the chip` | covered |
| A SHARED conversation shows the same chips as the chat (DR command, ask answers) instead of the raw marker text | unit  | `client/src/components/Share/__tests__/Message.chips.spec.tsx#renders the DR start chip instead of the raw marker` | covered |
| The DR chip caption follows the persisted `drKind`, never the message text | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/ActionChip.renderPaths.spec.tsx#takes the caption from drKind, not from the text` | covered |
...
| The ask card on the share page renders statically without chat hooks (no crash, no buttons) | unit  | `client/src/components/Chat/Messages/Content/__tests__/AskUserCall.spec.tsx#renders a STATIC card outside ChatContext` | covered |
