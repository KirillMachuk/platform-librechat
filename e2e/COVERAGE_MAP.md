...
| Stop before the first token leaves the question and no answer turn (screen, reload, database)   | e2e   | `e2e/specs/mock/stop-before-answer.spec.ts`                                                        | covered |
...
| A run that breaks after finishing a tool call is not announced as a failed request                 | e2e   | `e2e/specs/mock/run-incomplete.spec.ts#says the work is saved and how to carry on, not that the request failed` | covered |
| A tool call the run never reached states the outcome, not a cause                                  | unit  | `client/src/components/Chat/Messages/Content/Parts/__tests__/toolCallNotRun.test.tsx#does not claim anyone cancelled it` | covered |
| Code sees a file created after the sandbox already had files in it                                 | unit  | `packages/api/src/agents/handlers.sandboxSession.spec.ts#lets code see a file created AFTER the sandbox already had files in it` | covered |
...
| The reply's first token paints exactly where the waiting label stood — in-flow direct prose child (owner r24 п.1) | unit  | `client/src/components/Chat/Messages/Content/__tests__/MarkdownBlocks.test.tsx#renders the thinking indicator` | covered |
| A finished reply never keeps the waiting shimmer (AGENT_UPDATE tail, whitespace-only last part) | unit  | `client/src/components/Chat/Messages/Content/__tests__/Part.test.tsx#never keeps the placeholder` | covered |
| Reasoning expansion survives the finalization remount (r24 audit) | unit  | `client/src/components/Chat/Messages/Content/__tests__/ContentParts.autoExpand.test.tsx#re-seeds a user-expanded card` | covered |
| The measured thinking duration survives finalization and is wiped for a different message (К4) | unit  | `client/src/components/Chat/Messages/Content/__tests__/ContentParts.autoExpand.test.tsx#measures first→last tick` | covered |
| Auto-follow resumes when the user returns to the bottom on their own (r24 audit) | unit  | `client/src/hooks/Messages/__tests__/useMessageScrolling.spec.tsx#clears abortScroll when the end sentinel becomes visible` | covered |
| The composer never triggers the browser's contact-autofill popover (owner r24) | unit  | `client/src/components/Chat/Input/__tests__/ChatForm.spec.tsx#opts the composer out of browser autofill` | covered |
| One waiting word: streaming thoughts carry the shimmering «Думаю…» header (owner r24) | unit  | `client/src/components/Chat/Messages/Content/__tests__/Reasoning.label.spec.tsx#streams under the shimmering` | covered |
| Finished thoughts read «Думал N с» this session and plain «Мысли» after a reload (К4) | unit  | `client/src/components/Chat/Messages/Content/__tests__/Reasoning.label.spec.tsx#shows «Думал N с»` | covered |
| The thinking stream preview reveals clamped sentence rows and only grows (К4) | unit  | `client/src/components/Chat/Cards/__tests__/ThinkingReasoning.spec.tsx#reveals sentences as clamped rows` | covered |
| The thinking preview caps at 180px and auto-scrolls behind the fade masks (К4) | unit  | `client/src/components/Chat/Cards/__tests__/ThinkingReasoning.spec.tsx#caps the viewport at 180px` | covered |
| A reopened finished thinking block shows the FULL text, not the clamped preview (К4) | unit  | `client/src/components/Chat/Cards/__tests__/ThinkingReasoning.spec.tsx#finished: folds to the summary line` | covered |
| A thinking block mounted from history does not replay the entry animation (К4) | unit  | `client/src/components/Chat/Cards/__tests__/ThinkingReasoning.spec.tsx#mounted finished is static` | covered |
| Streaming thought rows are as tall as their text: one-line rows leave no empty line under them (р34) | unit  | `client/src/components/Chat/Cards/__tests__/ThinkingReasoning.spec.tsx#one-line rows take one line` | covered |
| A finished thought text is paragraphs per double newline, no empty line between them (р34) | unit  | `client/src/components/Chat/Cards/__tests__/ThinkingReasoning.spec.tsx#a finished text is paragraphs` | covered |
| The thinking header and the waiting label are the size of the message (р34, owner) | unit  | `client/src/components/Chat/Cards/__tests__/waitGateContract.spec.ts#that shared list carries the geometry` | covered |
| Thinking streams live: sentence rows under the shimmer, «Thought for Ns» fold, full text on reopen (К4) | e2e   | `e2e/specs/mock/thinking.spec.ts#streams sentence rows, folds into the duration line` | covered |
| No waiting label under the running research card — indicator and card strictly complementary (owner r24) | unit  | `client/src/components/Chat/Messages/Content/__tests__/ThinkingIndicator.drphase.spec.tsx#renders NOTHING for graph phases` | covered |
| ApprovalCard (cards К1): carousel auto-advance, «Другое…» commit, Continue gate, header ✕ plate | unit  | `client/src/components/Chat/Cards/__tests__/ApprovalCard.spec.tsx#keeps Continue disabled` | covered |
| DR plan card on the approval frame (К2, r30): header ✕ = cancel marker, no timer — a plan waits for a click; «run immediately» self-starts only a plan that just arrived live, never one from history; a draft or a refusal keeps the buttons | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/PlanCard.spec.tsx#starts itself once` | covered |
| A plan's FINAL marks it as just arrived before its card is committed — the one seam the self-start depends on (r30) | unit  | `client/src/hooks/SSE/__tests__/useEventHandlers.livePlan.spec.tsx#marks the plan as just arrived` | covered |
| «Запускать исследование сразу» in Settings → Chat: off by default, explanation on screen, hidden without the plan gate or the research permission (r30) | unit  | `client/src/components/Nav/SettingsTabs/Chat/__tests__/DeepResearchAutoStart.spec.tsx#is absent while the plan gate is off` | covered |
| ask_user tool end to end (К3): advertised to a plain run, card → selections → ONE summary chip → static card | e2e   | `e2e/specs/mock/ask-user.spec.ts#answers flow` | covered |
| An answer option keeps 4px of room for the focus ring on every side, first and last option (review 02.09 К1) | e2e   | `e2e/specs/mock/ask-user.spec.ts#an answer option has room for its focus ring on every side` | covered |
| Unsent ask_user answers (a picked option and a typed «Другое…») survive a page reload and Continue sends them; the draft is cleared once committed (owner, r26 follow-up) | e2e   | `e2e/specs/mock/ask-user.spec.ts#a half-filled card comes back the same after a reload` | covered |
| Mid-run steering: the composer stays a live channel while a research run gathers — Send instead of Stop, closed with a reason once the report is written | unit  | `client/src/components/Chat/Input/__tests__/ChatForm.steer.spec.tsx` | covered |
| Mid-run steering: a clarification joins the feed, the running answer moves under it, a toast confirms; a refusal shows the server's next step and keeps the text | unit  | `client/src/hooks/Chat/__tests__/useSteerRun.spec.tsx` | covered |
| Mid-run steering: clarifications survive the turn's final (slotted between the request and the answer, deduplicated after a reload) | unit  | `client/src/hooks/SSE/__tests__/useEventHandlers.midRunSteers.spec.tsx` | covered |
| Mid-run steering: Enter reaches Send only when the composer steers a running research; an ordinary generation still swallows it | unit  | `client/src/hooks/Input/__tests__/useTextarea.steer.spec.tsx` | covered |
| Mid-run steering: a plan card steered mid-way still owns the live run and finds its outcome below the clarifications | unit  | `client/src/components/Messages/__tests__/drRunWiring.spec.tsx#a finished run steered mid-way still shows its outcome` | covered |
| ask_user args validated and clamped; invalid questions do not consume the 1-3 cap | unit  | `packages/data-provider/specs/askUser.spec.ts#drops questions with fewer than 2 options` | covered |
| The answers card: one summary submit, refusal keeps it actionable, static when not latest / while streaming | unit  | `client/src/components/Chat/Messages/Content/__tests__/AskUserCall.spec.tsx#submits ONE summary message` | covered |
| The answers chip mounts only under a parent carrying an ask_user tool call (provenance anchor) | unit  | `packages/data-provider/specs/askUser.spec.ts#detects the ask_user tool call` | covered |
| DR command rows are hidden on BOTH user render paths (r25) — prod stores these commands without `content[]` | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/command.renderPaths.spec.tsx#hides the START row on the text-only path` | covered |
| A hidden DR command row stands aside for the editor, so the message's own Edit button is not a no-op | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/command.renderPaths.spec.tsx#yields to the editor when the message is being edited` | covered |
| Every surface that draws the user's own turn carries both chip rules, or is exempt with a stated reason | unit  | `client/src/components/Chat/Messages/ui/__tests__/userTurnSurfaces.guard.spec.ts#mounts both chip rules, or is exempt with a reason` | covered |
| A field that arrives late (drKind on the server save, a re-parented command) still reaches the render past React.memo | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/command.renderPaths.spec.tsx#drKind added after the first render hides the row` | covered |
| A SHARED conversation hides the DR start row but keeps everything BELOW it — progress, report (r25a review) | unit  | `client/src/components/Share/__tests__/Message.chips.spec.tsx#keeps the whole subtree under a hidden start row` | covered |
| A SHARED conversation shows the ask-answers chip and keeps the cancel row as prose | unit  | `client/src/components/Share/__tests__/Message.chips.spec.tsx#renders the ask_user answers chip under a tool-bearing parent` | covered |
| A hidden DR command row keeps its sibling switcher when the branch has siblings (r25a review) | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/command.renderPaths.spec.tsx#keeps the sibling switcher` | covered |
| A cancelled plan wears the «Отменено» badge on the card itself (r25) | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/PlanCard.spec.tsx#a cancelled plan wears the «Отменено» badge` | covered |
| ask_user options are selectable while the tail text streams; commit waits for the turn end (r25) | unit  | `client/src/components/Chat/Messages/Content/__tests__/AskUserCall.spec.tsx#options are selectable while the tail text still streams` | covered |
| Mid-stream ask_user selections survive the finalization remount (r25) | unit  | `client/src/components/Chat/Messages/Content/__tests__/ContentParts.autoExpand.test.tsx#re-seeds mid-stream selections` | covered |
| An answered/historical questions card folds into a one-line summary (r25) | unit  | `client/src/components/Chat/Messages/Content/__tests__/AskUserCall.spec.tsx#an answered card folds into the one-line summary` | covered |
| A folded questions card still SHOWS the chosen answers, from the answers message or the session draft (r25c) | unit  | `client/src/components/Chat/Messages/Content/__tests__/AskUserCall.spec.tsx#restores the chosen option from the answers MESSAGE` | covered |
| Answers picked WHILE the tail streams survive finalization, even when the part index shifts (owner r26) | unit  | `client/src/components/Chat/Messages/Content/__tests__/askUserSurvival.spec.tsx#keeps the answers when the part INDEX shifts` | covered |
| The model's closing sentence under the questions card is not drawn (owner r26) | unit  | `client/src/components/Chat/Messages/Content/__tests__/askUserSurvival.spec.tsx#the closing sentence under the card is not drawn` | covered |
| ONE waiting look: the streaming reasoning header IS the standalone waiting label, brain and all (owner r27) | unit  | `client/src/components/Chat/Cards/__tests__/ThinkingReasoning.spec.tsx#ONE waiting look` | covered |
| The questions options behave like a real radio group: one tab stop, arrows move within it (r25c) | unit  | `client/src/components/Chat/Cards/__tests__/ApprovalCard.spec.tsx#ArrowDown/ArrowUp move the focus inside the group` | covered |
| The running-step shimmer paints without owning layout, and no component rule repaints over it (r25 пакет Б review) | unit  | `client/src/components/Chat/Cards/__tests__/shimmerContract.spec.ts#the running step rule does NOT set a color` | covered |
| Enter in «Другое…» advances the carousel when the question is not the last (r25 acceptance) | unit  | `client/src/components/Chat/Cards/__tests__/ApprovalCard.spec.tsx#Enter in «Другое…» ADVANCES` | covered |
| The ask_user tool description teaches single-choice, one-subject questions and gapless ranges (r25 acceptance) | unit  | `packages/api/src/agents/tools.spec.ts#states that the card is single-choice` | covered |
| The plan handed to the graph is masked, and costs no second anonymizer call — it is read out of the already-masked question (r28) | unit  | `api/server/services/Endpoints/agents/deepResearchRun.spec.js#costs NO extra anonymizer call` | covered |
| Reading the plan out of a transcript does not swallow the clarify questions, which are numbered the same way (r28) | unit  | `packages/api/src/deepResearch/rebuild/plan.spec.ts#does NOT swallow the clarify questions` | covered |
| The live action line shows a masked sub-question as the generic «Исследует источники» — machinery never reaches the screen, and the progress channel stays synchronous (r28 chip) | unit  | `api/server/services/Endpoints/agents/deepResearchRun.spec.js#a masked sub-question never reaches the screen as machinery` | covered |
| A reused icon slot shows its glyph again for a new domain, instead of staying falsely loaded (r28 review) | unit  | `client/src/components/Web/__tests__/FaviconImage.test.tsx#shows the glyph again when the same slot is reused` | covered |
| One message, one node: an array that carries the same messageId twice does not grow a phantom branch in the thread (r29) | unit  | `packages/data-provider/src/messages.spec.ts#the same message twice is one node` | covered |
| A resumed turn's snapshot excludes the turn itself, so finishing it cannot append its own user message a second time (r29) | unit  | `client/src/hooks/SSE/__tests__/useResumeOnLoad.spec.tsx#the resumed snapshot does not carry the turn it resumes` | covered |
| The sibling switcher of a hidden Deep Research command sits in the chat column, on the user's side — not 40px left of it (owner r29) | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/CommandRow.spec.tsx#takes its width from the shared column helper` | covered |
| A plan step is always one line, so the two step extractors cannot disagree and drop the agenda (r28 review) | unit  | `packages/api/src/deepResearch/rebuild/plan.spec.ts#folds a multi-line step` | covered |
| A source favicon never leaves an empty hole beside its label — a glyph holds the box until the icon decodes, and stays if it never arrives (owner r28) | unit  | `client/src/components/Web/__tests__/FaviconImage.test.tsx#holds the box with a neutral glyph` | covered |
| A source favicon is fetched from our own backend, so the reader's browser never tells a third party which sites their research surfaced (owner r28) | unit  | `client/src/components/Web/__tests__/FaviconImage.test.tsx#asks our own backend for the icon` | covered |
| The favicon endpoint fetches a domain once and serves everyone else from cache, including a burst of icons rendering at the same moment | unit  | `packages/api/src/web/favicon.spec.ts#collapses a burst of concurrent readers into a single fetch` | covered |
| A domain that is an address, an internal name, or a smuggled URL never reaches the favicon endpoint's upstream | unit  | `packages/api/src/web/favicon.spec.ts#strips a smuggled path, port, credential, query or fragment` | covered |
| The favicon endpoint's upstream host is a constant and the requested domain only ever rides in the query — never in the destination | unit  | `packages/api/src/web/favicon.spec.ts#names the domain to upstream as a parameter, never as a destination` | covered |
| Source icons are fetched while the model writes, so the reader is not the one waiting for them | unit  | `packages/api/src/web/favicon.spec.ts#leaves nothing for the reader to wait for` | covered |
| Warming asks for the exact key the browser will request, `www.` and letter case included | unit  | `packages/api/src/web/favicon.spec.ts#warms the exact key the browser will ask for` | covered |
| Warming stays background work: six outbound requests at most across the whole process, however many searches overlap | unit  | `packages/api/src/web/favicon.spec.ts#never has more than six requests outstanding` | covered |
| Warming spends its budget on sites not yet held, so the ninth search of a research run still warms its new domains | unit  | `packages/api/src/web/favicon.spec.ts#spends its budget on the sites it does not have` | covered |
| A search hands over its own sources, not every source the request has accumulated | unit  | `api/server/services/Tools/search.spec.js#hands over the search that just happened` | covered |
| A search result hands its source links to the icon cache, and a fault there never costs the reader the sources | unit  | `api/server/services/Tools/search.spec.js#still streams the sources when warming the icons throws` | covered |
| The favicon route refuses a request with no session, so the icon endpoint is not an open proxy, and counts every request against a per-reader ceiling | unit  | `api/server/routes/__tests__/favicon.spec.js#the endpoint is not an open proxy` | covered |
| The favicon endpoint answers a missing, oversized, or non-image upstream reply with one stable «no icon» | unit  | `packages/api/src/web/favicon.spec.ts#treats an oversized answer as no icon` | covered |
| An icon request proves itself with the session cookie, since an `<img>` carries no Authorization header | unit  | `packages/api/src/web/favicon.spec.ts#lets a valid session through and names the user for the rate limiter` | covered |
| A PUBLICLY SHARED conversation is read without a session, so its source icons stay the neutral glyph instead of showing — the icon endpoint requires a cookie (owner's call, r28) | unit  | — | gap |
| The folded questions card is not clipped out of its own hairline edge by the collapse wrapper (owner r28) | unit  | `client/src/components/Chat/Messages/Content/__tests__/AskUserCall.spec.tsx#the clipping wrapper is widened` | covered |
| Text whose colour comes from a clipped gradient stays readable when selected — every zeroing class names a selection colour (owner r28) | unit  | `client/src/components/Chat/Cards/__tests__/shimmerContract.spec.ts#every one of them has a selection rule that RESTORES the fill` | covered |
| A gate-ON turn cannot reach the graph without a plan — an off-contract PROCEED fails closed to a card (r28 review) | unit  | `api/server/services/Endpoints/agents/deepResearchRun.spec.js#a gate-ON turn can no longer reach the graph without a plan` | covered |
| Answering the clarifying questions still gets a plan, never a planless run (r28 review) | unit  | `api/server/services/Endpoints/agents/deepResearchRun.spec.js#answering the clarifying questions still gets a plan` | covered |
| A continuation after Stop is drawn by the plan card — neither surface may go dark (r28 review) | unit  | `client/src/components/Messages/__tests__/drRunWiring.spec.tsx#a continuation after Stop is still drawn by the plan card` | covered |
| A research always gets a plan — a fresh turn may only clarify or plan (owner r28) | unit  | `packages/api/src/deepResearch/rebuild/plan.spec.ts#a FRESH turn may only clarify or plan` | covered |
| A continuation after Stop keeps the plan the user approved, instead of running with none (owner r28) | unit  | `api/server/services/Endpoints/agents/deepResearchRun.spec.js#a continuation after Stop keeps the plan` | covered |
| A run with no plan invents no steps — no «Шаги» well, only the action line and the bar (owner r28) | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/ProgressCard.spec.tsx#invents NO steps when a run has no plan` | covered |
| A run WITH a plan is drawn by the plan card itself — one card, live statuses, Stop in its header (owner r26) | unit  | `client/src/components/Messages/__tests__/drRunWiring.spec.tsx#the plan whose start command leads to the active tail` | covered |
| The standalone running card steps aside for a plan run and stays for a PROCEED run (r26) | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/RunningSlot.spec.tsx#steps aside when the snapshot carries plan steps` | covered |
| A second research does not also light up the first plan card — the NEAREST start command owns the run (r27) | unit  | `client/src/components/Messages/__tests__/drRunWiring.spec.tsx#a SECOND research does not also light up the first plan card` | covered |
| The live run is recognised on the message shape the CLIENT holds during it — `drKind` is stamped only at finalization (r27 review) | unit  | `client/src/components/Messages/__tests__/drRunWiring.spec.tsx#the SAME run, reloaded` | covered |
| The waiting label is hidden unless a `.submitting` ancestor is present, and shares ONE declaration list with the reasoning header (owner r27) | unit  | `client/src/components/Chat/Cards/__tests__/waitGateContract.spec.ts#the label is hidden by default` | covered |
| A re-plan turn does not light the stopped plan card — the run belongs to the turn whose USER message is the start command (r27) | unit  | `client/src/components/Messages/__tests__/drRunWiring.spec.tsx#a RE-PLAN turn does not light the stopped plan card` | covered |
| A STOPPED run says «Остановлено» and does not claim its steps are done (r26 review) | unit  | `client/src/components/Messages/__tests__/drRunWiring.spec.tsx#a stopped run says` | covered |
| The «Исследование отменено.» paragraph is hidden under the plan card's own badge; lookalike prose is not (owner r26) | unit  | `client/src/components/Messages/__tests__/drRunWiring.spec.tsx#the «Исследование отменено.» notice is not drawn` | covered |
| The highlighted plan step is the one the RUN reported, never derived from a progress fraction (owner r27; the fraction itself is gone since review 02.09 item 8) | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/RunFooter.spec.ts#the first research round is step 1, not step 3` | covered |
| The research card's line claims no fraction — an indeterminate 1px activity line with no `aria-valuenow`, paused while the run is parked offline (design review 02.09, item 8) | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/ProgressCard.spec.tsx#puts Stop in the header and claims no fraction` | covered |
| The research card counts the sources the researchers have actually cited, and says nothing at zero (design review 02.09, item 9) | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/ProgressCard.spec.tsx#counts the sources the researchers have actually cited` | covered |
| `dr_progress` carries the DISTINCT source count instead of a fraction: a findings event re-emits the same phase and action with the count grown, and the count persists into the report phase | unit  | `api/server/services/Endpoints/agents/deepResearchRun.spec.js#emits dr_progress snapshots (plan steps + current action) during a START research run` | covered |
| The engine surfaces a researcher batch's cited pages as a `findings` progress event | unit  | `packages/api/src/deepResearch/rebuild/run.spec.ts#streams the REPORT tokens and surfaces research progress` | covered |
| A run that reports NO step marks none — the card shows no active and no done step (owner r27) | unit  | `client/src/components/Messages/__tests__/drRunWiring.spec.tsx#a run that reports NO step marks none` | covered |
| SUPERVISOR receives the approved plan as its agenda and names the step each batch advances (owner r27) | unit  | `packages/api/src/deepResearch/rebuild/nodes/supervisor.spec.ts#puts the plan in the HUMAN material` | covered |
| A PROCEED run keeps the supervisor prompt it was measured on — no plan block, no planStep (r27) | unit  | `packages/api/src/deepResearch/rebuild/nodes/supervisor.spec.ts#a PROCEED run (no plan) gets the prompt it always had` | covered |
| The runner puts the reported step on the wire, monotonic, and closes the plan on the report phase (owner r27) | unit  | `api/server/services/Endpoints/agents/deepResearchRun.spec.js#carries the approved plan into the graph` | covered |
| The card header's icon-only action says what it does on hover and focus (owner r27) | unit  | `client/src/components/Chat/Cards/__tests__/ApprovalCard.spec.tsx#the header action carries its label as a visible plate` | covered |
| The finished report card wears the same frame as the plan it came from (owner r27) | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/ReportCard.spec.tsx#wears the shared card frame` | covered |
| A finished run leaves every plan step done; a plan that never ran keeps its resting look (r26) | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/PlanCard.spec.tsx#a finished run leaves every step done` | covered |
| The step being worked on shimmers and is never hidden behind «Ещё N» (r25 пакет Б) | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/PlanCard.spec.tsx#the running step shimmers, and is never hidden` | covered |
| Stop lives in the card header and progress is announced to assistive tech | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/ProgressCard.spec.tsx#puts Stop in the header` | covered |
| Offline freezes the merged card: no active step, no action line (r26) | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/PlanCard.spec.tsx#offline freezes it` | covered |
| The composer opts out of Chrome contact autofill with the honored token (r25) | unit  | `client/src/components/Chat/Input/__tests__/ChatForm.spec.tsx#opts the composer out of browser autofill` | covered |
| The shared-link projection carries `drKind`, without which the shared page loses the DR command chip and the truncated-report note | unit  | `packages/data-schemas/src/methods/share.test.ts#should carry drKind through to the shared projection` | covered |
| The ask card on the share page renders statically without chat hooks (no crash, no buttons) | unit  | `client/src/components/Chat/Messages/Content/__tests__/AskUserCall.spec.tsx#renders the folded line outside ChatContext` | covered |
...
| A generated deck accepts only bounded verified-PDF bytes and shows an honest failure when the asset is missing or invalid (never the lossy renderer) | unit | `client/src/components/Artifacts/__tests__/VerifiedPresentationPreview.test.tsx` | covered |
...
| First click navigates before a slow conversation-details request completes           | e2e   | `e2e/specs/mock/conversation-management.spec.ts#opens on the first click before details load`                              | covered |
| Open a conversation from the list                                                    | e2e   | `e2e/specs/mock/conversation-management.spec.ts`                                                                          | covered |
| Composer stays centered while a sidebar conversation loads                           | e2e   | `e2e/specs/mock/conversation-management.spec.ts#keeps the composer on one axis while a sidebar chat loads`                 | covered |
...
| A triple-click copies exactly the message text — the paragraph break after it is not part of the selection (useTrimSelectionEnd) | e2e | `e2e/specs/mock/message-actions.spec.ts#a triple-click copies the message text without a tail of blank lines` | covered |
| The last message in the chat copies the same way (its selection used to run into the composer strip) | e2e | `e2e/specs/mock/message-actions.spec.ts#the last message in the chat copies the same way` | covered |
| A drag released below a message copies the message, not the blank space under it | e2e | `e2e/specs/mock/message-actions.spec.ts#a drag released below the message copies the message` | covered |
| A drag from the last message released over the composer copies exactly the message (the release target is not a guard) | e2e | `e2e/specs/mock/message-actions.spec.ts#a drag from the last message released over the composer copies exactly the message` | covered |
| Inside a code block a newline is content: a copied middle code line keeps it (the block's last line ends without one — Chromium) | e2e | `e2e/specs/mock/message-actions.spec.ts#a code line keeps its newline` | covered |
| A message copied while the composer holds the caret is still exactly the message (the focus guard was the defect) | e2e | `e2e/specs/mock/message-actions.spec.ts#a message copied while the composer holds the caret is still exactly the message` | covered |
| A question answered with a thinking block copies exactly the question — the triple-click ends inside the «Думаю» header, past its icon, and the icon is not a reason to keep the tail (the picture rule was the defect) | e2e | `e2e/specs/mock/message-actions.spec.ts#a question answered with a thinking block copies exactly the question` | covered |
| A plan awaiting approval is shown whole — nothing behind «Ещё N»; the preview well applies once the plan is a record (design review 02.09, item 10) | unit | `client/src/components/Chat/Cards/__tests__/ApprovalCard.spec.tsx#a plan awaiting approval is shown whole` | covered |
| On a phone the «Thoughts» header, «Ещё N» under a plan (once the plan is a record) and the ✕ that cancels a research reach 44px to a finger without covering a neighbour's centre (design review 02.09, item 6) | e2e | `e2e/specs/mock/card-tap-targets.spec.ts#the «Thoughts» header, «Ещё N» under a plan and the cancel ✕` | covered |
| On a phone the folded questions card's toggle reaches 44px to a finger (design review 02.09, item 6) | e2e | `e2e/specs/mock/card-tap-targets.spec.ts#the folded questions card` | covered |
| The report reader's table of contents highlights under a cursor only, never under a finger (design review 02.09, item 7) | unit | `client/src/components/Chat/Messages/DeepResearch/__tests__/ReportCard.spec.tsx#the table of contents highlights under a cursor only, never under a finger` | covered |
| Copying from the message editor copies the field's own text and leaves its selection alone | e2e | `e2e/specs/mock/message-actions.spec.ts#copying from the message editor copies the field` | covered |
| The trimming logic itself: paragraph break, pre-wrap, `<pre>`, a picture or an icon in the tail is not a reason to keep it, tables as text, inert blocks, form controls, direction, never collapsing | unit | `client/src/utils/__tests__/selectionEnd.spec.ts#a paragraph selected up to the start of the next block ends at its last character` | covered |
