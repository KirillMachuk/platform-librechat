...
| A run WITH a plan is drawn by the plan card itself — one card, live statuses, Stop in its header (owner r26) | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/PlanCard.spec.tsx#the approved plan itself shows the live statuses` | covered |
...
| The step being worked on shimmers and is never hidden behind «Ещё N» (r25 пакет Б) | unit  | `client/src/components/Chat/Messages/DeepResearch/__tests__/ProgressCard.spec.tsx#never hides the running step` | covered |
