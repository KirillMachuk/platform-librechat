...
| A domain resolving inside our own network is refused by the favicon endpoint without a fetch | unit  | `packages/api/src/web/favicon.spec.ts#refuses a domain that resolves inside our own network` | covered |
