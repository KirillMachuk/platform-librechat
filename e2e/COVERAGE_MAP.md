...
| A conversation read through a share link keeps its source icons, because the icon route does not demand a session | unit  | `api/server/routes/__tests__/favicon.spec.js#serves a reader with no session` | covered |
| A stranger cannot time our cache to learn whether the client searched a site: a reader with no session is served from a store of its own | unit  | `packages/api/src/web/favicon.spec.ts#does not answer a stranger from the store warming fills` | covered |
| The handler picks that store by whether the cookie named a reader — the seam the separation rests on | unit  | `packages/api/src/web/favicon.spec.ts#serves a reader the cookie did not name from the store share links fill` | covered |
| An anonymous flood cannot evict the icons the client's own answers depend on: the two stores have separate budgets | unit  | `packages/api/src/web/favicon.spec.ts#gives the store behind share links a smaller budget` | covered |
| A reader who proves nothing is counted against their address, and a forged cookie buys no bucket of its own | unit  | `api/server/routes/__tests__/favicon.spec.js#treats a forged session as no session` | covered |
| With sharing switched off the icon route refuses a reader with no session, as there is no public page to be on | unit  | `api/server/routes/__tests__/favicon.spec.js#refuses an unnamed reader when sharing is switched off` | covered |
...
| A signed-in reader is still named when sharing is switched off, so turning sharing off costs nobody their icons | unit  | `packages/api/src/web/favicon.spec.ts#still names a signed-in reader when sharing is switched off` | covered |
| An icon request names its reader from the session cookie when there is one, since an `<img>` carries no Authorization header | unit  | `packages/api/src/web/favicon.spec.ts#names the reader from a valid session` | covered |
