...
| The trimming logic itself: paragraph break, pre-wrap, `<pre>`, images in the tail, tables as text, inert blocks, form controls, direction, never collapsing | unit | `client/src/utils/__tests__/selectionEnd.spec.ts#a paragraph selected up to the start of the next block ends at its last character` | covered |
