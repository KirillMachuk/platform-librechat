 ...
 *
 * This runs against `e2e/config/librechat.permissions.yaml`, which switches
 * three permissions off and leaves the rest on. Why a whole second profile
...
test.describe('role permissions gate the interface', () => {
  test('the config seeded the role exactly as written', async ({ page }) => {
    ...
    expect(seeded.PROMPTS?.USE).toBe(false);
  ...
  test('a permission switched off takes its control with it', async ({ page }) => {
    ...
    await expect(page.getByTestId('sidebar-link-prompts')).toHaveCount(0);
  ...
  test('the permissions left on keep their controls', async ({ page }) => {
     ...
     * three above are not, the difference is the permission and nothing else. */
