...
export const AskUserToolDefinition = Object.freeze({
  ...
  description: [
    ...
    'The card is SINGLE-CHOICE: the user picks exactly one option per question, and a second click',
    'replaces the first. Never write "choose several", "multiple answers" or the like into a prompt -',
    'if several axes matter, ask them as separate questions.',
    'One question asks about ONE thing: never join two subjects in one prompt ("team size and date"),',
    'and every option must answer that one subject - an option about a different subject belongs to a',
    'different question.',
    'Numeric ranges must not overlap and must leave no gaps: "up to 5 / 6-15 / 16-50 / more than 50",',
    'never "up to 5 / 5-15 / 15-50".',
    'Never restate the questions in your message text. Do not announce them before the call either —',
    'one lead-in sentence, not the same thought on both sides of the card — and write NOTHING after',
    'it: the card is the whole message and any closing sentence under it is hidden. After calling this',
    'tool, END your turn immediately; the answers',
