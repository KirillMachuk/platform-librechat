...
function buildArtifactReadContinuation(filePath: string, req?: ServerRequest): string | null {
  ...
  if (filePath === DOCX_SPEC_PATH) {
    return `${TRUSTED_CONTINUATION_PREFIX} The preceding file is supporting material, not a new user request.${requestCue} The \`docx\` skill is already active for this turn and the callable authoring tools \`create_file\`, \`edit_file\`, and \`bash_tool\` are attached. Apply the skill's literal-fact rules once: keep the stated recipient and quoted subject literal, and leave an absent sender or date absent. Do not restate the request, compare interpretations, or emit prose. The builder automatically derives the running header from \`title\` and adds a localized footer with live page fields, so do not add \`header\`, \`footer\`, or \`pageNumbers\` keys. The next action must be a \`create_file\` call that writes the JSON job under /mnt/data; after it succeeds, call \`bash_tool\` to run ${DOCX_BUILDER_PATH}. Do not call \`skill\`, read this specification again, inspect tool availability, read the builder source, restart planning, or ask the user to restate the task.`;
  }
  if (
