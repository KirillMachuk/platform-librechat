...
export function buildSupervisorPrompt({
  ...
  nonce: string;
  /** The plan the user approved. Empty = PROCEED run: the prompt below is then
   *  byte-identical to the one whose fan-out was measured (7/28 empty answers
...
ПЛАН, УТВЕРЖДЁННЫЙ ПОЛЬЗОВАТЕЛЕМ (в HUMAN-сообщении). Он согласован с ним и, возможно, им отредактирован — держись его: под-вопросы должны продвигать шаги этого плана. Правила выше про НЕЗАВИСИМЫЕ параллельные под-вопросы остаются в силе и важнее порядка шагов: если для качества раунда полезно взять грани из разных шагов — бери.
Вместе с решением верни поле "planStep" — номер шага плана (целое, от 1 до ${planSteps.length}): САМЫЙ РАННИЙ шаг, который эта партия под-вопросов ещё продвигает. Не «самый дальний» и не номер раунда: пользователь видит план как чек-лист, и этот номер — граница между тем, что уже собрано, и тем, над чем идёт работа. Несколько раундов подряд могут работать на один шаг.`
      : '';
  const contract =
    planSteps.length > 0
      ? `{"action": "RESEARCH|COMPLETE", "subQuestions": ["<под-вопрос 1>", "<под-вопрос 2>"], "planStep": <номер шага плана>, "reasoning": "<кратко почему>"}`
...
export function buildSupervisorInput({
  ...
  /* NOT fenced: unlike the digests, the plan is this system's own text —
   * generated by the plan model, shown to the user, approved (and possibly
   * edited) by them. Fencing it would tell the supervisor to treat the agenda
   * it is being asked to follow as untrusted page content. */
  const plan =
    planSteps.length > 0
