...
ПЛАН, УТВЕРЖДЁННЫЙ ПОЛЬЗОВАТЕЛЕМ (в HUMAN-сообщении). Он согласован с ним и, возможно, им отредактирован — веди исследование по нему: под-вопросы каждого раунда должны продвигать шаги этого плана, по порядку, не перескакивая вперёд, пока текущий шаг не обеспечен материалом. Если бриф требует чего-то, чего в плане нет, — собери это в рамках ближайшего подходящего шага.
Вместе с решением верни поле "planStep" — номер шага плана (целое, от 1 до ${planSteps.length}), который продвигает ЭТА партия под-вопросов. Это номер шага, а не номер раунда: несколько раундов подряд могут работать на один шаг.`
      : '';
  const contract =
    planSteps.length > 0
