...
Вместе с решением верни поле "planStep" — номер шага плана (целое, от 1 до ${planSteps.length}), который эта партия под-вопросов продвигает БОЛЬШЕ ВСЕГО. Это номер шага, а не номер раунда: несколько раундов подряд могут работать на один шаг.`
      : '';
  const contract =
    planSteps.length > 0
