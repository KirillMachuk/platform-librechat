...
export function getRagRerankConfig(env: NodeJS.ProcessEnv = process.env): RagRerankConfig | null {
  ...
  return {
    ...
    // 16, не 36: живой прод-замер (qsr, v2-m3 int8, все 8 ядер) = ~0.55с/чанк, т.е. пул 36
    // (сервер клампит до RERANKER_MAX_DOCUMENTS=24) = ~13.5с ≫ таймаута → КАЖДЫЙ реранк молча
    // истекал и откатывался на дистанцию (фича была фактически выключена и на file_search, и на
    // library). 16 чанков = ~7.4с, укладывается в таймаут. Выигрыш реранка идёт с верха пула
    // (промахи в dense-топ-5), поэтому усечение 36→16 сохраняет переупорядочивание; глубокий
    // отбор из большого пула жертвуется ради латентности на CPU. Больше — только на GPU-реранкере.
    candidates: intEnv(env.RAG_RERANK_CANDIDATES, 16, 2, 64),
    // 10000: пул 16 = ~7.4с одиночно; под конкурентной нагрузкой реранкер сериализует, поднимать
