...
const BookmarkItem: FC<MenuItemProps> = ({ tag, selected, handleSubmit, icon, ...rest }) => {
  ...
  const renderIcon = () => {
    ...
    if (selected) {
      return <BookmarkFilled aria-hidden="true" className="size-4" />;
    }
  ...
  return (
    <MenuItem
      ...
      className="focus-inset group flex w-full gap-2 rounded-lg p-2.5 text-sm text-text-primary transition-colors duration-200 focus:outline-none data-[focus]:bg-surface-hover"
      {...rest}
      as="button"
