...
class ArtifactJobTests(unittest.TestCase):
    def test_docx_skill_enforces_bounded_llm_agnostic_authoring(self):
        ...
        self.assertIn("the quoted phrase is the document title or subject", skill)
        self.assertIn("the first assistant action must be the single documented `read_file` call", skill)
        self.assertIn("Do not do semantic parsing until those reads finish", skill)
        self.assertIn("`ФИО`, `имя`, `зовут`, or `по имени`", skill)
        self.assertNotIn("служебная записка для генерального директора", skill)
        self.assertIn("Unmarked common nouns remain concepts", skill)
