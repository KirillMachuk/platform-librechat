...
9. If a defect remains, revise the JSON and rerun in the same turn. A report with `status: "needs_review"`, any failed QA check, or any critical issue is not a deliverable and must never be described to the user as complete. Allow at most two repair iterations and set `repairIterations` to the actual count.
