...
`outputPdf` defaults to `true`: the normal result is a same-stem editable PPTX and PDF. Set the literal boolean `false` only when the user explicitly asks for PPTX alone; in that case the builder creates an internal `.preview.pdf` for render QA.
...
- `previewAssets[]` exposes the verified render used by the product preview. `delivery` is normally `requested` because both same-stem formats are user deliverables. It is `preview_only` only after an explicit PPTX-only request. The server adds the authenticated `filepath`; the skill must never supply it.
